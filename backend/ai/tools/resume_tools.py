...
@tool
def parse_resume_file(file_content: str, file_type: str) -> dict[str, Any]:
    """Parse structured resume data from PDF or text file content.

    Uses LangChain's TextLoader/PyPDFLoader, CharacterTextSplitter, and FAISS vector embeddings
    ...
    Args:
        file_content: Raw text extracted from the resume file
    ...
    # Convert string content to bytes for the loader
    file_bytes = file_content.encode("utf-8") if isinstance(file_content, str) else file_content

    # Load and split document using LangChain components
    chunks = _load_and_split_document(file_bytes, file_type)
    ...
    extracted = {
        ...
        "skills": _extract_skills(vector_store, chunks),
        "experience": _extract_experience(vector_store, chunks),
        "education": _extract_education(vector_store, chunks),
        "certifications": _extract_certifications(vector_store, chunks),
    }
